...
SELECT job_role, AVG(job_satisfaction) AS average_satisfaction
FROM hrdata
GROUP BY job_role
ORDER BY average_satisfaction DESC;
...
-- 13. Job Satisfaction vs. Attrition (Scatter Plot Insights)
...
SELECT marital_status, AVG(job_satisfaction) AS average_satisfaction
FROM hrdata
GROUP BY marital_status;

-- 15. Attrition Rate by Job Role and Age Band
SELECT job_role, age_band, 
       SUM(CASE WHEN attrition = 'Yes' THEN 1 ELSE 0 END) / COUNT(*) * 100 AS attrition_rate
FROM hrdata
GROUP BY job_role, age_band
ORDER BY attrition_rate DESC;
